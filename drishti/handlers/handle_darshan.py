...
def handler():
    try:
        ...
        if not hasattr(sys.modules[__name__], 'args'):
            raise ValueError("No args provided to handle_darshan. Please set handle_darshan.args before calling handler().")
        
        # Check that the file exists before proceeding
        if not os.path.isfile(args.log_path):
            print(f"Darshan file not found: {args.log_path}")
            return {}
        ...
        if 'STDIO' in report.records:
            ...
            if df_stdio:
                ...
                total_write_size_stdio = 0
                total_read_size_stdio = 0
                total_size_stdio = 0
        else:
            df_stdio = None
            total_write_size_stdio = 0
        ...
        if 'POSIX' in report.records:
            ...
            if df_posix:
                ...
                total_write_size_posix = 0
                total_read_size_posix = 0
                total_size_posix = 0
        else:
            df_posix = None
            total_write_size_posix = 0
        ...
        if 'MPI-IO' in report.records:
            ...
            if df_mpiio:
                ...
                total_write_size_mpiio = 0
                total_read_size_mpiio = 0
                total_size_mpiio = 0
        else:
            df_mpiio = None
            total_write_size_mpiio = 0
        ...
        #export_csv(filename, job['job']['jobid'])
            # Build a dictionary containing all counters
        final_counters = {
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python handle_darshan.py /path/to/file.darshan")
        sys.exit(1)

    log_file = sys.argv[1]
    mpiio_counters = handler()
    print("MPI-IO counters from handle_darshan.py:", mpiio_counters)
"""
